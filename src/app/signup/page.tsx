...
export default function SignupPage() {
  ...
  const handleAuthSuccess = async (user: FirebaseUser) => {
    ...
    try {
        ...
        // Step 2: Call the server action to check for and assign the admin role.
        await setAdminRole(user.uid, false); // `false` is a default, server will override if it's the first user
        
        // Step 3: Force a token refresh to get the new custom claim if it was assigned.
        ...
        toast({ variant: 'destructive', title: 'Post-signup failed', description: error.message || 'Could not finalize your account setup.' });
    }
  };
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We must first update the auth user's profile with the name
      await updateProfile(userCredential.user, { displayName: name });
      
      // Pass the original user object from the credential, which has all methods.
